#!/bin/bash
# Script de Rescate: Solución MB_LEN_MAX y finalización de Fase 2
...
echo "🛠️ [PARCHE] ARREGLANDO LIMITS.H DEL COMPILADOR..."
tar -xf gcc-13.2.0.tar.xz
LIMIT_DIR=$(dirname $($LFS_TGT-gcc -print-libgcc-file-name))
mkdir -p $LIMIT_DIR/install-tools/include
mkdir -p $LIMIT_DIR/include-fixed

# Fusionamos los límites de GCC con los del sistema
cat gcc-13.2.0/gcc/limitx.h gcc-13.2.0/gcc/glimits.h gcc-13.2.0/gcc/limity.h > $LIMIT_DIR/install-tools/include/limits.h
cat gcc-13.2.0/gcc/limitx.h gcc-13.2.0/gcc/glimits.h gcc-13.2.0/gcc/limity.h > $LIMIT_DIR/include-fixed/limits.h
rm -rf gcc-13.2.0

echo "🧹 Limpiando el intento fallido de Coreutils..."
rm -rf coreutils-9.4 2>/dev/null || true

echo "🚀 REANUDANDO FASE 2 DESDE COREUTILS..."

# ==========================================
# 2.6. Coreutils (ls, cat, mkdir, etc.)
# ==========================================
echo ">>> [3/5] Compilando Coreutils..."
